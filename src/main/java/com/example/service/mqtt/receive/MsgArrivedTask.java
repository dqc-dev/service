...
public class MsgArrivedTask implements Runnable {
    ...
    public void run() {
        ...
        String topicPrefix = topic.substring(0, (topic.lastIndexOf("/") + 1));

        BaseTopicService baseTopicService = (BaseTopicService) SpringUtil.getBean(topicPrefix);

        try {
            baseTopicService.processMessageArrived(topic, (byte[]) message.getPayload());
        } catch (Exception e) {
            e.printStackTrace();
        }
